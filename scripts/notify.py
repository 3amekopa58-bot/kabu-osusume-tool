...
# （2026-09-19に1,557銘柄・26年で取り直した。944銘柄版は三菱UFJ等が
#  欠けた壊れたユニバースだった＝4.4-61。旧々値は日経225・5年536件で
#  測ったもので、到達率が2〜3pt高く出ていた）：
#   直近高値      目標+1.6%  到達率88.4%  ← 目標が近すぎて意味をなさない
#   ATR×3        目標+7.1%  到達率62.6%  ← 採用
#   フィボ127.2%  目標+7.4%  到達率61.2%
#   固定+10.3%    目標+10.3% 到達率49.9%
...
# （2026-09-19に1,557銘柄ユニバース・26年19,150トレードで取り直した）：
#   〜4%: 67.5%(206件) / 4-6%: 68.8%(1,207件) / 6-8%: 63.6%(1,468件)
#   8-10%: 58.4%(791件) / 10%〜: 51.9%(1,148件)
#  944銘柄版（59.1 / 54.9）より高い帯で到達率が下がった。
# 日経225の225銘柄で測っていた旧値（71/67/63/55）より低い。
...
# 損切りと期待値は backtest.py の採用ルールを過去26年19,150トレードで集計した値。
# **売買ルールを変えたときも、対象銘柄リストを変えたときも必ず取り直すこと**：
#   python3 backtest.py timesl either trend marketadx volume rs sl10 max --tickers universe.csv
#   その後 python3 audit_notify_constants.py で突き合わせる（4.4-56）
#
# ⚠️ **平均は少数の大勝ちに強く引っ張られる。** 13,646件の中央値は
#    **+0.63%** しかなく、平均+3.14%との差は大きい。実際、8105.T の
#    1トレード（+1,189%）だけで平均が+0.09pt動く。
#    「期待値+3.0%」は**平均的なトレードの姿ではない**ことに注意。
STOP_HIT_RATE = 31.5       # 損切り-10%に到達した割合（実測31.52%）
EXPECTED_PCT = 3.43        # 1トレードあたりの期待値（平均リターン）
MEDIAN_PCT = 0.43          # 中央値。平均との差が大きいので併記に使う

# ⚠️ **上の2つは取引コストを引く前の数字**（4.4-67）。
#    backtest.py の cost_pct は既定0で、正規コマンドにも付いていない。
#    流動性に応じたコスト模型（スプレッド＋平方根則のインパクト、
#    注文100万円）で見積もると往復の中央値は0.28%。それを引くと：
#      平均 +3.43% → +2.92% ／ 中央値 +0.43% → **+0.03%＝ほぼ±0**
#    PFは1.77→1.62。**利益は裾の大勝ちに依存しており、
#    典型的な1トレードはコストで消える。** ここは利用者に伝える。
#    ⚠️ コストは推定であって実測ではない（インパクト係数c=0.5の仮定）。
#       c=1.0なら中央値は-0.19%になる。
NET_EXPECTED_PCT = 2.92    # コスト控除後の平均
NET_MEDIAN_PCT = 0.03      # コスト控除後の中央値
COST_MEDIAN_PCT = 0.28     # 往復コストの中央値（推定）

# ◇参考枠（日経がADX20超でない、または100日線より下の日）の実測値。
# 26年42,960トレードを相場環境で分けて測った（REQUIREMENTS 4.4-34 / 4.4-61）：
#   本命 16,618件 勝率50.8% 平均+3.45% PF1.77
#   参考 26,342件 勝率47.4% 平均+1.35% PF1.30
# ⚠️ 「裏付けが弱い」ではなく**明確に劣る**。ただし重複しない3期間のうち
#    第2期（2010-2018）だけは参考のほうが良く（PF1.84 vs 1.41）、
#    常に劣るわけではない。数字を示して判断は利用者に委ねる方針。
PARTIAL_WIN_RATE = 47.4
PARTIAL_EXPECTED_PCT = 1.35
PARTIAL_PF = 1.30
...
# 印がいくつ付いているかで成績が変わることを26年5,868トレードで実測した
# （REQUIREMENTS 4.4-33 / 2026-09-19に1,557銘柄ユニバースで取り直し）。
# 印は互いに独立していて、重ねるほど良くなる：
#   0個 PF1.33 勝率49.5% / 1個 2.15 54.0% / 2個 2.91 62.7% / 3個 19.40 88.2%
# ⚠️ **3個そろうのは51件（全体の0.9%）しかない。** PF19.40 は極端な
#    少数例に依存しており、そのまま期待してはいけない。944銘柄版では
#    128件・PF8.47 だった＝件数が減って数字だけ跳ね上がっている。
# ⚠️ 第1期（2000-2010）は0件のままで、**印の検証は26年ではなく実質16年**。
MARK_STATS = {0: (1.33, 49.5), 1: (2.15, 54.0), 2: (2.91, 62.7), 3: (19.40, 88.2)}
...
def format_marks(r) -> str:
    ...
    return f"\n  印{n}個 → 過去の実測で勝率{wr:.0f}%・PF{pf:.2f}（全体は52.7%・1.72）"
...
def build_message() -> str:
    ...
    if partial:
        parts.append(f"◇参考（条件を一部満たす）※本命より成績が劣る（下の実測を参照）")
        for r in partial[:PICK_COUNT]:
            parts.append(format_pick(r, names) + "\n" + missing_note(r))
        parts.append(
            f"　※◇参考枠の実測（26年26,342件）: 勝率{PARTIAL_WIN_RATE:.1f}%・"
            f"平均+{PARTIAL_EXPECTED_PCT:.2f}%・PF{PARTIAL_PF:.2f}。"
            f"◆本命（51.6%・+3.09%・PF1.71）より明確に劣る。"
    ...
    # だが、screen.py は毎日ゼロから全銘柄を評価するため保有状況を知らない。
    # 実測では通知の30%が前日以前と同じ銘柄だった（REQUIREMENTS 4.4-35）。
    # 4.4-43 でその買い増しを実測した：予算100万円では100株単位の制約で
    # 増し玉がほとんど発生せず（取引数+5%・3回目以降は26年で0件）、
    ...
    footer = (
        ...
        # ⚠️ 平均だけを出すと「1回あたり+3%くらい」と受け取られるが、
        # 中央値は+0.6%しかない（4.4-56）。典型的なトレードの姿が
        # 伝わるよう中央値を併記する
