...
# （2026-09-05に944銘柄・26年で取り直した。旧値は日経225・5年536件で
...
# （2026-09-05に944銘柄ユニバース・26年13,645トレードで再照合。
#  帯ごとの実測は 0-6%:68.6% / 6-8%:63.3% / 8-10%:59.1% / 10%〜:54.9% で、
#  下の設定値と1pt以内で一致することを確認済み）：
#   〜4%: 68.7%(134件) / 4-6%: 68.6%(866件) / 6-8%: 63.3%(1,020件)
#   8-10%: 59.1%(543件) / 10%〜: 54.9%(612件)
...
# 損切りと期待値は backtest.py の採用ルールを過去26年13,646トレードで集計した値。
...
STOP_HIT_RATE = 30.4       # 損切り-10%に到達した割合（実測30.37%）
EXPECTED_PCT = 3.14        # 1トレードあたりの期待値（平均リターン）
MEDIAN_PCT = 0.63          # 中央値。平均との差が大きいので併記に使う

# ◇参考枠（日経がADX20超でない、または100日線より下の日）の実測値。
# 26年30,304トレードを相場環境で分けて測った（REQUIREMENTS 4.4-34）：
#   本命 11,835件 勝率51.6% 平均+3.09% PF1.71
#   参考 18,468件 勝率48.3% 平均+1.47% PF1.33
# ⚠️ 「裏付けが弱い」ではなく**明確に劣る**。ただし重複しない3期間のうち
#    第2期（2010-2018）だけは参考のほうが良く（PF1.84 vs 1.41）、
#    常に劣るわけではない。数字を示して判断は利用者に委ねる方針。
PARTIAL_WIN_RATE = 48.3
PARTIAL_EXPECTED_PCT = 1.47
PARTIAL_PF = 1.33
...
def build_message() -> str:
    ...
    if partial:
        ...
        parts.append(
            f"　※◇参考枠の実測（26年18,468件）: 勝率{PARTIAL_WIN_RATE:.1f}%・"
